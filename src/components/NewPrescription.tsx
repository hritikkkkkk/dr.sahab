...
// Initialize with a fallback to prevent crash on module load if env var is missing
...
export default function NewPrescription({ user, setScreen }: { user: AuthUser, setScreen: (s: Screen) => void }) {
  ...
  const handleCreatePatient = async () => {
    ...
    try {
      ...
      setAllPatients(prev => [p as any, ...prev]);
      setShowAddPatient(false);
      localStorage.setItem('current_patient', JSON.stringify(p));
    }
  };
  ...
  const toggleMic = () => {
    if (isRecording) return; // Simplified stop handling
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-10 pb-24">
          ...
          {!selectedPatient ? (
              ...
              {!showAddPatient ? (
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <input type="text" placeholder="Weight (kg)" value={newWeight} onChange={e => setNewWeight(e.target.value)} className="w-1/3 px-4 py-3 bg-zinc-50 border border-zinc-100 rounded-xl text-sm font-medium outline-none focus:border-zinc-950" />
                  </div>
                  <div className="flex gap-2">
                    <input type="text" placeholder="Height (cm)" value={newHeight} onChange={e => setNewHeight(e.target.value)} className="w-1/2 px-4 py-3 bg-zinc-50 border border-zinc-100 rounded-xl text-sm font-medium outline-none focus:border-zinc-950" />
                    <div className="w-1/2 flex gap-2">
                      <input type="text" placeholder="BP" value={newBp} onChange={e => setNewBp(e.target.value)} className="w-1/2 px-4 py-3 bg-zinc-50 border border-zinc-100 rounded-xl text-sm font-medium outline-none focus:border-zinc-950" />
                      <input type="text" placeholder="Temp" value={newTemp} onChange={e => setNewTemp(e.target.value)} className="w-1/2 px-4 py-3 bg-zinc-50 border border-zinc-100 rounded-xl text-sm font-medium outline-none focus:border-zinc-950" />
                    </div>
                  </div>
            ...
            <div className="clinical-card p-6 relative overflow-hidden group">
              ...
              <div className="space-y-3 mb-8">
                <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 mb-1">Current Vitals (Optional)</p>
                <div className="flex gap-2">
                  <input type="text" placeholder="Weight (kg)" value={newWeight} onChange={e => setNewWeight(e.target.value)} className="w-1/2 px-4 py-2 bg-zinc-50 border border-zinc-100 rounded-lg text-xs font-medium outline-none focus:border-zinc-950" />
                  <input type="text" placeholder="Height (cm)" value={newHeight} onChange={e => setNewHeight(e.target.value)} className="w-1/2 px-4 py-2 bg-zinc-50 border border-zinc-100 rounded-lg text-xs font-medium outline-none focus:border-zinc-950" />
                </div>
                <div className="flex gap-2">
                  <input type="text" placeholder="BP (120/80)" value={newBp} onChange={e => setNewBp(e.target.value)} className="w-1/2 px-4 py-2 bg-zinc-50 border border-zinc-100 rounded-lg text-xs font-medium outline-none focus:border-zinc-950" />
                  <input type="text" placeholder="Temp (°C)" value={newTemp} onChange={e => setNewTemp(e.target.value)} className="w-1/2 px-4 py-2 bg-zinc-50 border border-zinc-100 rounded-lg text-xs font-medium outline-none focus:border-zinc-950" />
                </div>
              </div>
            ...
            <div className="relative group mb-6">
              ...
              <div className="absolute bottom-6 right-6 flex gap-3">
                ...
                <button 
                  ...
                  onClick={toggleMic}
                  className={`p-3 border rounded-2xl transition-all shadow-lg hover:scale-110 ${isRecording ? 'bg-red-50 border-red-200 animate-pulse' : 'bg-white border-zinc-100 hover:border-zinc-950'}`}
                >
                  <Mic size={20} className={isRecording ? 'text-red-600' : 'text-zinc-400'} />
                </button>
              </div>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-8 bg-white/70 backdrop-blur-2xl border-t border-zinc-100 flex justify-end gap-6 z-40">
        <button 
          onClick={() => alert("Intelligence Draft Saved successfully!")}
          className="px-10 py-4 rounded-2xl border border-zinc-200 text-xs font-bold uppercase tracking-[0.2em] hover:bg-zinc-50 transition-all"
        >
          Save Intelligence Draft
        </button>
        <button 
          onClick={() => {
            if (!selectedPatient && newName) {
              const p = { 
                id: 'temp-' + Date.now(), 
                name: newName, 
                age: newAge || '30', 
              ...
              const p = {
                ...selectedPatient,
                weight: newWeight || selectedPatient.weight,
                height: newHeight || selectedPatient.height,
                bp: newBp || selectedPatient.bp,
                temp: newTemp || selectedPatient.temp
              };
              localStorage.setItem('current_patient', JSON.stringify(p));
            }
            localStorage.setItem('current_diagnosis', diagnosis);
            setScreen('PRESCRIPTION_BUILDER');
          }}
          className="px-10 py-4 rounded-2xl bg-red-600 text-white text-xs font-bold uppercase tracking-[0.2em] flex items-center gap-3 shadow-2xl hover:bg-red-700 active:scale-95 transition-all"
        >
