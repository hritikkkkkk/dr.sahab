...
export default function PrescriptionBuilder({ user, setScreen }: { user: AuthUser, setScreen: (s: Screen) => void }) {
  ...
  useEffect(() => {
    const fetchContext = async () => {
      ...
      const cachedDiagnosis = localStorage.getItem('current_diagnosis');
      if (cachedDiagnosis) {
  ...
  const searchMedicines = async () => {
    ...
    try {
      ...
      // Fallback mock data if permissions fail
      setMedResults([
        { id: 'mock1', name: medQuery + ' Mock 1', salt: 'Demo Salt', type: 'TAB' },
        { id: 'mock2', name: medQuery + ' Mock 2', salt: 'Demo Salt', type: 'SYP' }
      ]);
    }
  };
  ...
  const addMedication = () => {
    ...
    if (selectedMeds.length > 0) {
      names = selectedMeds.map(m => m.name);
    } else if (medQuery.trim()) {
      names = medQuery.split(',').map(n => n.trim()).filter(n => n);
    }
  ...
  return (
    <div className="h-full flex overflow-hidden bg-zinc-50 print:bg-white">
      {/* Pane 1: Context (Left) */}
      <aside className="w-[300px] flex flex-col gap-6 shrink-0 h-full p-6 overflow-y-auto border-r border-zinc-100 print:hidden">
        {/* Patient Detail Card */}
        <div className="clinical-card p-6 flex flex-col items-center text-center">
          <div className="relative mb-4">
            <div className="w-20 h-20 bg-zinc-100 rounded-full border-4 border-white shadow-sm flex items-center justify-center text-3xl font-serif font-bold text-zinc-400">
          ...
          <div className="w-full mt-6 pt-4 border-t border-zinc-50 grid grid-cols-2 gap-4 text-left">
            ...
            <div>
              ...
              <p className="text-xs font-mono font-bold">{patient.weight || '--'} kg / {patient.height || '--'} cm</p>
            </div>
          </div>
        </div>
        ...
        <div className="clinical-card p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-zinc-50 p-3 rounded-lg border border-zinc-100">
              ...
              <p className="text-sm font-mono font-bold text-red-600">{patient.bp || '--'}</p>
            </div>
            <div className="bg-zinc-50 p-3 rounded-lg border border-zinc-100">
              <div className="flex items-center gap-1 text-[8px] font-bold text-zinc-400 uppercase mb-1">
                <Thermometer size={10} /> Temp
              </div>
              <p className="text-sm font-mono font-bold">{patient.temp || '--'}<span className="text-[10px] ml-0.5 opacity-40">°C</span></p>
            </div>
          </div>
        </div>
      ...
      <section className="flex-1 flex flex-col min-w-0 bg-white shadow-[0_0_10px_rgba(0,0,0,0.05)] z-10 print:hidden">
          ...
          <section className="space-y-6">
            ...
            {selectedMeds.length > 0 && (
              <div className="p-4 bg-red-50 border border-red-100 rounded-xl flex flex-col gap-2">
                <p className="text-xs font-bold text-red-600 uppercase tracking-widest mb-1">Selected Drugs</p>
                <div className="flex flex-wrap gap-2">
                  {selectedMeds.map(med => (
                    <div key={med.id} className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-red-100 shadow-sm">
                      <span className="font-bold text-zinc-950">{med.name}</span>
                      <button onClick={() => setSelectedMeds(selectedMeds.filter(m => m.id !== med.id))} className="text-red-500 hover:text-red-700 transition-colors">
                        <X size={14} />
                      </button>
                    </div>
                  ))}
            ...
            {/* Dose Config Grid */}
            <div className="p-5 bg-zinc-50 border border-zinc-100 rounded-2xl space-y-5">
               <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
                 ...
                 <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <input 
                        ...
                        placeholder="500mg"
                        className="flex-1 px-3 py-2.5 border border-zinc-200 rounded-lg text-xs font-mono font-bold bg-white outline-none focus:border-zinc-950 transition-colors" 
                      />
                      <select 
                        value={dosageConfig.type}
                        onChange={e => setDosageConfig({...dosageConfig, type: e.target.value})}
                        className="w-24 bg-white border border-zinc-200 rounded-lg px-2 text-[10px] font-bold outline-none focus:border-zinc-950 transition-colors"
                      >
                 ...
                 <div className="space-y-2">
                    ...
                    <div className="flex border border-zinc-200 rounded-lg overflow-hidden bg-white focus-within:border-zinc-950 transition-colors">
                      <input 
                        type="number" 
                        value={dosageConfig.duration} 
                        onChange={e => setDosageConfig({...dosageConfig, duration: e.target.value})}
                        className="w-full px-3 py-2.5 text-center font-mono font-bold outline-none" 
                      />
                      <span className="bg-zinc-50 px-4 border-l border-zinc-200 text-[10px] font-bold flex items-center text-zinc-500">DAYS</span>
                    </div>
                 </div>
               </div>
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                    {frequencies.map(f => (
                      <button 
                        ...
                        className={`py-2.5 border rounded-lg text-[10px] font-bold transition-all ${dosageConfig.frequency === f ? 'border-zinc-950 bg-zinc-950 text-white shadow-md' : 'border-zinc-200 bg-white text-zinc-600 hover:border-zinc-400 hover:bg-zinc-50'}`}
                      >
                        {f}
                      </button>
                    ))}
               ...
               <div className="space-y-2">
                  ...
                  <div className="flex gap-3">
                    <select 
                      ...
                      className="flex-1 bg-white border border-zinc-200 rounded-lg px-3 py-2.5 text-xs font-bold outline-none focus:border-zinc-950 transition-colors"
                    >
               ...
               <div className="pt-2">
                 <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-white p-4 rounded-xl border border-zinc-200 shadow-sm">
                    ...
                    <button 
                      disabled={selectedMeds.length === 0 && !medQuery.trim()}
                      onClick={addMedication}
                      className="w-full xl:w-auto px-6 py-3 bg-red-600 text-white rounded-xl font-bold text-xs shadow-lg hover:bg-red-700 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
                    >
                      <Plus size={16} /> Append to Rx
                    </button>
                 </div>
               </div>
            </div>
      ...
      <aside className="w-[450px] shrink-0 h-full p-8 flex flex-col bg-zinc-100">
        <div className="flex-1 bg-white shadow-2xl rounded-sm p-12 relative overflow-hidden flex flex-col border-t-8 border-red-600">
          ...
          <div className="relative z-10 flex flex-col h-full">
            <div className="flex justify-between items-start border-b border-zinc-200 pb-6 mb-8">
              <div>
                <h2 className="font-serif text-3xl font-black leading-none mb-1">
                  {user?.name || (user?.email ? `Dr. ${user.email.split('@')[0].split(/[._]/).map(s => s.charAt(0).toUpperCase() + s.slice(1)).join(' ')}` : 'Dr. Sahab')}
                </h2>
        ...
        <div className="mt-6 flex gap-3 print:hidden">
          <button onClick={() => window.print()} className="p-3 bg-white border border-zinc-200 rounded-xl hover:bg-zinc-50 transition-colors flex items-center justify-center"><Printer size={20} /></button>
