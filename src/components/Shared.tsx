...
export function Sidebar({ currentScreen, setScreen }: { currentScreen: Screen, setScreen: (s: Screen) => void }) {
  ...
  return (
    <nav className="hidden md:flex flex-col h-screen w-64 fixed left-0 top-0 bg-white border-r border-zinc-100 shadow-[4px_0_20px_rgba(0,0,0,0.05)] py-6 z-40 print:hidden">
...
export function Navbar({ user, setScreen }: { user?: AuthUser, setScreen: (s: Screen) => void }) {
  return (
    <header className="fixed top-0 left-0 right-0 h-16 bg-[#111111] text-white z-50 flex items-center justify-between px-6 md:pl-[17rem] print:hidden">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right hidden lg:block">
            <p className="text-sm font-bold text-white leading-tight">
              {user?.name || (user?.email ? `Dr. ${user.email.split('@')[0].split(/[._]/).map(s => s.charAt(0).toUpperCase() + s.slice(1)).join(' ')}` : 'Dr. Sahab, MD')}
            </p>
            <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Internal Medicine</p>
          </div>
          <div className="w-9 h-9 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center font-serif font-bold text-white shadow-sm">
            {user?.name ? user.name.charAt(0).toUpperCase() : (user?.email ? user.email.charAt(0).toUpperCase() : 'D')}
          </div>
        </div>
      </div>
    </header>
...
export function MobileNav({ setScreen }: { setScreen: (s: Screen) => void }) {
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-zinc-100 flex items-center justify-around z-50 print:hidden">
